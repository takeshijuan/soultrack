...
- `app/layout.tsx` — `UserButton` + `Footer` added; `clientMessages` excludes legal keys from client bundle (~8KB/request)
- `app/track/[id]/page.tsx` — `auth()` + `getTrack()` parallelized via `Promise.all`; `SaveToLibraryButton` with `initialSaved` state
- `i18n/request.ts` — updated merge pipeline: `en.json + legal-en.json` → `baseMessages`; `ja` locale additionally merges `legal-ja.json`
...
- `app/layout.tsx` — Added `suppressHydrationWarning` to fix next-auth hydration mismatch in App Router
